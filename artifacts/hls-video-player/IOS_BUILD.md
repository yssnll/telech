...
une playlist HLS, le serveur API sélectionne la meilleure variante et la convertit
en MP4 compatible iOS avant que l’app ne l’enregistre localement. L’app lit ensuite
uniquement le fichier local, sans réseau.

Le domaine public de l’application doit être fourni dans `EXPO_PUBLIC_DOMAIN` (ou
dans le champ `api_domain` du workflow GitHub Actions) et doit exposer l’API sous
`/api`. Cela est nécessaire pour convertir un flux HLS en MP4 pendant le
téléchargement. Les vidéos apparaissent ensuite dans l’onglet **Hors ligne**.
