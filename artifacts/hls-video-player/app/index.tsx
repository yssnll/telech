...
type OfflineVideo = {
  ...
  createdAt: number;
  size: number;
  // Kept for backwards compatibility with downloads created before the MP4 fix.
  format: 'mp4' | 'hls';
};
...
function getApiBaseUrl() {
  ...
  if (!configuredDomain) {
    throw new Error('Le domaine de conversion vidéo n’est pas configuré pour cette app.');
  }

  return configuredDomain.startsWith('http://') || configuredDomain.startsWith('https://')
    ? configuredDomain.replace(/\/+$/, '')
    : `https://${configuredDomain}`;
}

function getConvertedMp4Url(sourceUrl: string) {
  const endpoint = new URL('/api/downloads/mp4', getApiBaseUrl());
  endpoint.searchParams.set('url', sourceUrl);
  endpoint.searchParams.set('mode', 'compatible');
...
export default function PlayerScreen() {
  ...
  const downloadHlsAsMp4 = async (
    sourceUrl: string,
    directory: Directory,
    onProgress: (value: number) => void,
  ) => {
    // A local m3u8 file is not reliably playable by AVPlayer on iOS. The API
    // resolves the HLS variants and muxes the stream into a self-contained MP4.
    onProgress(5);
    const result = await File.downloadFileAsync(
      getConvertedMp4Url(sourceUrl),
      new File(directory, 'video.mp4'),
      { idempotent: true },
    );

    if (!result.exists || result.size <= 0) {
      throw new Error('La conversion MP4 a produit un fichier vide ou indisponible.');
    }
  ...
  const downloadSource = async (requestedSourceUrl?: string) => {
    ...
    const filename = `${getFilename(sourceUrl)}.mp4`;
    const directory = new Directory(Paths.document, OFFLINE_DIRECTORY, id);
    setDownloadState('working');
    setDownloadProgress(0);
    setDownloadMessage(isHlsUrl(sourceUrl) ? 'Conversion et téléchargement de la vidéo…' : 'Téléchargement de la vidéo…');
    ...
    try {
      ...
      if (isHlsUrl(sourceUrl)) {
        setDownloadMessage('Conversion et téléchargement de la vidéo…');
        const result = await downloadHlsAsMp4(sourceUrl, directory, setDownloadProgress);
        localUri = result.uri;
        size = result.size;
        format = 'mp4';
      } else {
  ...
  const playOfflineVideo = async (video: OfflineVideo) => {
    ...
    if (isLegacyHls) {
      Alert.alert(
        ...
        'Cette vidéo a été enregistrée dans un ancien format qui n’est pas fiable sur iOS. Téléchargez-la à nouveau pour obtenir un fichier MP4 lisible hors ligne.',
        [
          { text: 'Plus tard', style: 'cancel' },
          {
            text: 'Retélécharger en MP4',
            onPress: () => {
